...
int main() {
	...
	for (rep = 0; rep < 4; rep++) {
		...
		while (validKey <= maxValue) {
			...
			if (!strcmp(encrypted_text, encoded_text)) {
				for (i = 0; i < npes; i++) if (i != myrank) MPI_Isend(&endAllThreads, 1 MPI_INT, i, 99, MPI_COMM_WORLD);
				end_flag = 1;
		...
		// printf("Chave nao encontrada pelo no %d.", myrank);
		return 0;
	}
}
